...
#Define $f(x)=\exp(-x^2)$. Suppose we are interested in the following integral \[I :=\int_a^b f(x) dx\] and to estimate using Monte-Carlo simulation techniques.

a function that draws n (uniformly distributed) random numbers in the rectangle defined by $x=[a,b]; y = [0,\underset{x\in[a,b]}{\max}f(x)]$ and calculates an estimate of I. Note that a strategy for calculating I is to use
...
```
#Plot the estimate against n and the result from the build-in function *integrate()* to the figure. 
```{r}
...
# Moments 
calculation of the partial variances defined by \[\hat\sigma^2_j = \frac{1}{j}\sum_{k=1}^j (x_k-\overline{x}_k)^2\] 
 the plot these partial variances against n.
